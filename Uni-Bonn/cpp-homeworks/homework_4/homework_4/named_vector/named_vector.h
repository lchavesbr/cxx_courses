...
struct named_vector {
  ...
  named_vector() : name_{}, std_vector_{} {};
  ...
  void reserve(std::size_t new_capacity) { std_vector_.reserve(new_capacity); };
  void resize(std::size_t new_size) { std_vector_.resize(new_size); };
  [[nodiscard]] std::size_t capacity() const { return std_vector_.capacity(); };
};
